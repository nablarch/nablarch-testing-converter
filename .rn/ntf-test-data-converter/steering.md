...
- [ ] **§6-F の残り（カラム名の重複。ユーザー判断待ち）**: `ColumnRowDataBlock` に同じ番人を置いて全テストを回したところ `YamlFormatReaderInvalidInputTest#dropsValueWhenTableColumnNamesDifferOnlyByCase` が落ちた（実測 2026-08-19）。**辺②は重複したカラム名を持つ中間モデルを実際に作る**（**YML-10**。本体 `TableData` がテーブル系のカラム名を大文字化するため、`id` と `ID` を書いたスキーマ適合の YAML が `columnNames` ＝ `[ID, ID]` になる）。生成時に拒否すると NTF 自身が読めるこの入力を変換できなくなるため、番人は入れていない。**`issues.md` XLS-40 の方針 4「辺②は構造上重複しないので呼び出し側のバグ」は事実に反していたので訂正した。** 選択肢は (a) 番人を入れて YML-10 の入力を変換エラーにする ／ (b) 辺②で大文字化後の衝突を検出して WARN する（YML-10 の「判断」欄が求める扱い。辺①の F1-05 と同型） ／ (c) カラム名側を別課題として切り出し保留にする
