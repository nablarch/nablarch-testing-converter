...
- 本作業で見つかった現状挙動の課題が課題一覧へ記録されている。**#25.5 で要対応と判定した 7 件は修正され、それ以外は修正されずに記録のみである**（ユーザー確定・2026-08-14 の方針変更に合わせて改訂。改訂前は「修正されずに」が全件に掛かっていた。**当初 5 件 → XLS-22 を追加して 6 件（2026-08-18）→ yaml 側の修正が入った YML-03 を追加して 7 件（2026-08-18）**）
...
- 1 task = 1 commit
- （フェーズ1）実装の変更は一切しない。package/import の機械的調整と pom 設定のみ許可。**フェーズ2 の #25.5 だけがこの例外で、要対応 7 件の修正に限り src/main を変更する**（ユーザー確定・2026-08-14。**件数は 5 → 6 → 7 と増えた。下の行を参照**）
- 本体・yaml には書き込まない
...
- **本作業で見つかった不具合は、要対応と判定した 7 件（YML-02・YML-12・XLS-16・XLS-06・YML-08・XLS-22・YML-03）に限り修正する。それ以外は従来どおり記録のみ**（当初 5 件・ユーザー確定 2026-08-14。**XLS-22 を追加・ユーザー確定 2026-08-18**。**YML-03 を追加・ユーザー指示 2026-08-18**——帰属先の nablarch-testing-yaml が `0b53910` で直ったため）。記録先は `.rn/ntf-test-data-converter/coverage/issues.md`。修正対象の判定根拠と手順は Decisions「不具合修正の対象と手順（#25.5）」
...
7 件に限り修正する**。それ以外は従来どおり記録のみである。Rules の該当 2 行と Acceptance criteria の
2 行はこの決定に合わせて改訂済み。
...
**Purpose**: 要対応と判定した 6 件（**2026-08-18 に YML-03 を追加して 7 件**）を TDD で修正し、不具合の挙動を緑のアサートで固定している状態を解消する。判定と出典は Decisions「不具合修正の対象と手順（#25.5）」。
...
- [ ] **§1-D（辺③④）**: `FieldDef.name` が `null` のフィールドを弾く。**明文確認の結果、番人は保留した（`7e4525a`。`src/main`・`src/test` は無変更）。ユーザー判断待ち。** 解説書 `testdata_notation.rst`（`30a8271`）の明文は `fields` の個別要素の名称が `null` である形まで届いていない（`:888` の「未指定または空」の主語は**リスト**、`:889` はサイズの話、`:871`／`:1060` は Excel の版面に `null` と空文字を区別するセルの状態が無く境界を決められない、`:1140` は「形」であって必須要件ではない）。**届いているのは本体スキーマ `$defs.field_def` だけ**（`required` ＝ `["name","type"]`／`name` ＝ `{"type": "string"}`。`minLength` が無いので `name: ""` は適合 ＝ 番人の境界と一致）。**未決 2 点** —— (1) 本体スキーマだけを根拠に番人を置いてよいか (2) 置くなら辺④だけか、辺③にも置くか（スキーマが直接掛かるのは YAML。Excel にこれに当たる明文は無い）。根拠は `issues.md` XLS-31 の「§1-D の明文確認」節
- [ ] **§1-E（辺③④）**: `TestDataBlock.groupId` が `null` のブロックを弾く（現状どちらの辺にも番人が無い）。**明文確認の結果、番人は保留した（`11643e7`。`src/main`・`src/test` は無変更）。ユーザー判断待ち。** 解説書 `:254` が定めるのは「省略が正当な状態であること」だけで `null` には触れず、書ける形を定める `:198`（`データタイプ=識別子の値`）・`:278`（`データタイプ名[グループID]=`）・`:330`（YAML は `group_id:` フィールド）も `null` と空文字を区別しない（どちらも「何も書かない」という同じ 1 つの出力になる）。**本体スキーマも届かない** —— `group_id` は `table_data`／`file_data`／`expected_request_message_data`／`group_message_data` の 4 定義とも `{"type": "string", "minLength": 1}` の**任意キー**であり、`null` を「キーを置かない」と写せば適合する（**§1-D より根拠が弱い**。§1-D は `name` が `required` だったのでスキーマだけは届いていた）。**ただし現状維持は選べない** —— 辺③の現状出力 `SETUP_TABLEnull=T` は `:198`・`:278` の示す形に無い。**未決（ユーザー判断）: 是正を (a) 弾く に倒すか (b) `null` を省略（空文字）へ正規化するか。**明文はどちらも許すため明文からは決まらない。根拠は `issues.md` XLS-32 の「§1-E の明文確認」節
...
- 7 件それぞれについて、仕様どおりの期待値のテストが存在し、**修正前に赤になったこと（失敗メッセージ）が記録されている**（YML-03 の待機テスト 2 件は #25.5 で赤を記録済み。`checks/task-25.5.md` の実行出力）
- 7 件それぞれについて、現状挙動を固定していた既存テストが削除されている（同じ挙動を主張するテストが 2 本残っていない）
- 1 件 ＝ 1 コミットになっている（`@Ignore` 付与のコミットを含め、混ぜていない）
- ~~YML-03 の `@Ignore` テストが存在し、理由が `@Ignore` の引数に書かれている~~ → **2026-08-18 に YML-03 を修正したため、`@Ignore` は 0 件であること**（`grep -rn '^    @Ignore' src/test --include=*.java` がヒット 0 件）に置き換えた
...
- `mvn clean test -Djacoco.skip=true` が全テスト PASS する（~~`@Ignore` は Skipped として現れてよい~~ → YML-03 修正後は `Skipped: 0` であること）
- `src/main` の変更が 7 件の修正に必要な範囲に限られており、変更したファイル名・課題ID・変更理由が記録されている
- `FieldDef.type` ／ `MessageDataBlock.records` の契約が Javadoc に明記され、辺③（`XlsFormatWriter`）と辺④（`YamlFormatWriter`）の双方が `IllegalArgumentException` で弾く。現状挙動を固定していたテストは置き換えられている（2 本残っていない）
- `issues.md` の `notation:nnn` が全件 `30a8271` 基準であり、基準コミットが本文に書かれている
- `RecordLayout` コンストラクタに番人を置かない判断と、却下理由・実測が `issues.md` に残っている
