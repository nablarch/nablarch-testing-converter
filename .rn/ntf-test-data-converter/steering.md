...
- [x] Acceptance criteria（フェーズ2分を含む全項目）を1件ずつ検証し、結果をまとめる → **検証は済み（2026-08-21）。13 項目のうち 12 項目が充足、1 項目が条件つき充足。検証不能は 0 件。**
  - **「全移動ファイルが本体現ブランチと package/import を除いて完全一致（実装無改変）」は充足である**（2026-08-21 に移動元 SHA を特定して照合し直した。全文は `checks/task-28.md`）。
    ...
    - **照合の射程**: 移動時点までである。`2a069bc..HEAD` の `src/main` 変更 79 コミット（#5・#11〜#17・#25.5・#26.5 ほか）は対象外で、AC が挙げる除外（#25.5・#26.5）はこの区間にある。本体側 `src/test` も対象外。
    - **持ち越し（要ユーザー判断・本作業では手を付けていない）**: `1035207` は reflog 依存であり、`gc.reflogExpireUnreachable` 既定 30 日に対して作成から約 2 か月が経過している。**gc が走れば消える。** 救出用のタグ・ブランチを本体に作るかは未決。
