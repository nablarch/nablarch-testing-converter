...
- **Status**: paused
- **Date**: 2026-08-21
- **Last completed**: #26.5 マーカーカラムのセル値を `[EMPTY]` へ改める（`0672a16`）。**#27 は進行中で未完了**（`axis-matrix.md` は `50c4a38` まで。軸E の ❌ 2 件は道 2 で解消済み ＝ `783810b` ／ `6d12021` ／ `50c4a38`）
- **Next**: #27 の続き。**直近は水平展開の着手可否のユーザー判断**（見積りは #27 の Steps に実測値で記録済み。246 セル・新規に読むテスト本文 126 件・おおよそ 4〜7 時間）
- **Notes**: branch `ntf-test-data-converter`。**ユーザー回答待ちは解消した** —— 軸E の ❌ 2 件は道 2（#27 の中でテスト 2 本を足して埋める）で確定し、実施済み（全件 `Tests run: 597, Failures: 0, Errors: 0, Skipped: 2`）。**#27 の残りは 5 つ** —— (a) **水平展開**（全セルについて「テスト本文が当該セルの主張する要素を実際にアサートしているか」を確かめる。見積り報告済み・着手可否はユーザー判断）、(b) `issues.md` の通読確認、(c) #26 からの持ち越し 8 件（用語・体裁。**対象は `coverage-report.md`** であって `axis-matrix.md` ではない。内容の正は `checks/task-26.md`「追加 1 巡」節）、(d) `checks/task-27.md` の self-check、(e) **3 者レビューの 3 巡目**（2 巡目の指摘を `832a700` で直したところまで。**Verify の反復上限は 3 巡で、これが最後の 1 巡になる**）。**2 巡のレビューで繰り返し出た欠陥型は「照合していない範囲について完全列挙・唯一性を断定する」であり、3 巡目でも同じ型を最初に探すこと。**
