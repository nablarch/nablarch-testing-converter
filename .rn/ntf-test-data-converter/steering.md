...
- [x] 表に記したテストメソッド名が実在することを、テストソースと突き合わせて確認する → **§0.6 の照合コマンド（抽出 313／照合対象 286／NG 0）。略記 `#メソッド名` とテストヘルパも解決して照合する。3 者のレビュアが独立に照合して NG 0 を再現済み**
...
- **Status**: paused
- **Date**: 2026-08-21
- **Last completed**: #26.5 マーカーカラムのセル値を `[EMPTY]` へ改める（`0672a16`）。**#27 は進行中で未完了**（成果物 `axis-matrix.md` は `832a700` まで push 済み）
- **Next**: #27 の続き（Steps の未チェック 7 件）
- **Notes**: branch `ntf-test-data-converter`（push 済み・ローカル差分なし・未追跡パス無し）。**ユーザー回答待ちが 1 件ある** —— 軸E の ❌ 2 件の扱い（#27 の Steps に本文を移してあるのでそちらを読むこと）。**#27 の残りは 4 つ** —— (a) `issues.md` の通読確認、(b) #26 からの持ち越し 8 件（用語・体裁。**対象は `coverage-report.md`** であって `axis-matrix.md` ではない。内容の正は `checks/task-26.md`「追加 1 巡」節）、(c) `checks/task-27.md` の self-check、(d) **3 者レビューの 3 巡目**（2 巡目の指摘を `832a700` で直したところまで。**Verify の反復上限は 3 巡で、これが最後の 1 巡になる**）。`axis-matrix.md` は `e977824` → `1c3253b` → `832a700` の 3 コミットで 1095 行超。**2 巡のレビューで繰り返し出た欠陥型は「照合していない範囲について完全列挙・唯一性を断定する」であり、3 巡目でも同じ型を最初に探すこと。**
