...
- **Status**: paused
- **Date**: 2026-08-24
- **Last completed**: **上流 yaml 振る舞い変更の現状把握**（是正なし・steering.md へのタスク番号追加なし・実測結果はチャット本文に報告済み）。`nablarch-testing-yaml` は `dd8f313`（`fb58781` 全値null／空文字行のスキップ、`6cbc4e1` 先頭空マッピング行でも列名解決、いずれも祖先に含む）が install 済み。`JAVA_HOME=/usr/lib/jvm/temurin-17-jdk-amd64 mvn -o clean test -Djacoco.skip=true` → **Tests run: 605, Failures: 8, Errors: 1, Skipped: 2**。失敗9件は `RoundTripTest.java:664`／`YamlFormatReaderInvalidInputTest.java:601,628`／`YamlFormatReaderScalarTest.java:352,364,505,572,584`／`YamlFormatWriterTest.java:660`（全件、判定＝**旧挙動固定**。converter 側の新規欠陥は無し。根拠は `YamlSection.java:183-209`（`dropBlankRows`／`fb58781`）と `:228-236`（`resolveColumns`／`6cbc4e1`）。`YamlFormatReader#nonMarkerColumns`（`YamlFormatReader.java:496-504`）は `dropBlankRows` を経由せず `resolveColumns` を直に呼ぶため list_map の columnNames と rows で通る経路が分かれる）。`issues.md` **YML-04**（5形）は `6cbc4e1` で**2形が変化**（先頭行が空マッピング `{}` のテーブル／list_map ケース＝`dropsAllRowsWhenFirstRowOfTableIsEmptyObject`・`keepsRowCountButLosesValuesWhenFirstRowOfListMapIsEmptyObject`。columnNames が `[]` → `[A]` へ）、**3形は不変**（`dropsColumnThatAppearsOnlyInSecondRowOfTable`・`dropsColumnThatAppearsOnlyInSecondRowOfListMap`・`padsColumnMissingFromSecondRowWithNullInTable`。先頭行が非空のため `6cbc4e1` の条件に該当せず PASS のまま）。YML-04 の核（先頭行だけでカラム集合が決まる）は未修正。
- **Next**: **是正方針をユーザーが判断してから指示を受ける**。候補は (a) 9件のテストを新挙動へ改定（TDD）するか、(b) `issues.md` YML-04 の判定・掲載を6cbc4e1の変化に合わせて改訂するか。**このセッションでは実施しない**（範囲外／レビュー不要の実測タスクとして完結）
- **Notes**: branch `ntf-test-data-converter`（push 不要・本タスクはコード変更ゼロ・作業ツリークリーン）。`nablarch-testing-yaml` 側も読み取りのみで無変更。**未決 2 件（#31 から持ち越し、未解消）** —— (1) 台帳の宣言値のずれ 2 件の出所が未確認（`XlsFormatWriterTest` の `build` ＋3 ／ `YamlFormatReaderInvalidInputTest` 3 区間目の ＋2 のうち #31 以外の 2 件。表は `checks/task-31.md`「台帳の宣言値のずれ」）／(2) 申し送りの束 4 件（XLS-27・XLS-39・XLS-40・XLS-42）は未提出で、出す判断は調整側。**マージ可否の判断は出さない**（Rules）
