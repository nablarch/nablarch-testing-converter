...
- **Notes**: branch `ntf-test-data-converter`（`d26d941` push 済み・作業ツリーはクリーン・未追跡パス無し）。**未決 2 件** —— (1) 台帳の宣言値のずれ 2 件の出所が未確認（`XlsFormatWriterTest` の `build` ＋3 ／ `YamlFormatReaderInvalidInputTest` 3 区間目の ＋2 のうち #31 以外の 2 件。表は `checks/task-31.md`「台帳の宣言値のずれ」）／(2) 申し送りの束 4 件（XLS-27・XLS-39・XLS-40・XLS-42）は未提出で、出す判断は調整側。**マージ可否の判断は出さない**（Rules）
