...
- **Status**: in progress
- **Date**: 2026-08-13
- **Last completed**: #22 辺③ 軸D・軸F（`d9b03f6`）。台帳の構造見直し（`c126856` ＋ `3f9e665`）。#23 は実装＋修正ラウンド2（`63c3f9b`）まで完了だが**未チェックオフ**
- **Next**: #23 のレビュー（QA・Craft・Verification）を `3f9e665` に対して再実行し、PASS ならチェックオフ。その後 #24（辺②）へ
- **Notes**: ブランチ `ntf-test-data-converter` / PR #1 https://github.com/nablarch/nablarch-testing-converter/pull/1。428 件全 PASS・`src/main` 無変更。
  **台帳の方針は決着済み（2026-08-13・ユーザー指示）**: `coverage/inventory.md` から逆引き表（§1.2／§2.2／§3.2／§4.2）・訂正履歴（§3.1-4／§3.1-5）・他ファイルの行番号・ファイル行数を削除した（1967 → 1324 行）。逆引きの正は #27 の `coverage/axis-matrix.md`。規約は Rules（フェーズ2）に 2 項追加し、#24 の Steps に self-check を入れた。この見直しは削除のみのためレビューは回していない。
  **#23 の残り**: QA・Craft はラウンド2 で FAIL のまま、Verification は `4905838` で PASS 済み。レビュー対象は台帳の構造見直し後の `3f9e665`。修正ラウンドは 3 回上限のうち 2 回を消化。
  ...
  **`checks/task-23.md`**: Self-check 列・Evidence 列・Method の適用のみ記入済み。**レビュー結果欄は空**で、チェックオフ時にコーディネータが埋める。
  **未決（ユーザー確認待ち・作業はブロックしない）**: 指示【1】の「件数はコマンドを併記した形だけを残す」を、既存の全 268 行へ遡って適用するかは未実施。今回は行番号とファイル行数のみ削除し、既存の件数はそのまま残した（遡及適用は削除では済まず書き換えになるため）。#24 以降の新規記述には self-check で規約を効かせる。
