...
**現況（2026-08-19・`issues.md` 冒頭の導出コマンドで確認）**: 全 52 件・**要対応 23 ／ 対応不要 28 ／
本作業の対象外 1**。要対応 23 件の内訳は **修正済み 15 件**（XLS-06・XLS-08・XLS-16・XLS-20・XLS-22・
XLS-28・XLS-29・XLS-30・XLS-31・XLS-32・XLS-33 ／ YML-02・YML-03・YML-08・YML-12）と
**未完 8 件**（XLS-27 は当面の対応まで完了・本体修正待ち。**XLS-34〜XLS-38・XLS-40・XLS-41 の 7 件は
§6 の中間モデル一巡点検で新たに起こしたもので未着手**）。内訳は `issues.md` 冒頭にも書いてある。
**2026-08-18 の 全 44 件・15 ／ 28 から動いた理由は 2 つある** —— XLS-20 の判定を「対応不要」から
「要対応」へ変えたこと（`73297e2`）と、**§6 で 8 件（XLS-34〜XLS-41）を新規に起こしたこと**である。
...
> 辺③の番人のまま。XLS-27 のマーカーカラム案の実測待ちのため）。上表の「番人の要否・揃える先」欄は
...
- [x] 中間モデルの全クラス・全フィールドを一巡し、「両形式が表現できない値を中間モデルが保持できる」箇所が他に無いかを点検して結果を記録する（無ければ「無し」と明記する）。**観点をもう 1 つ足す**（§6・ユーザー確定 2026-08-18）——**辺①が本体（`nablarch-testing`）経由で記法に無い形を中間モデルへ持ち込む経路が無いか**。XLS-08（マーカーカラムだけのブロックが「カラム 0 個・行 2 件」で入る）がその 1 例目である → **完了（2026-08-19）。結果は `coverage/issues.md` の「§6 中間モデル一巡点検（2026-08-19）」節**（一巡表 ＋ 新規課題 8 件 XLS-34〜XLS-41）。**「無し」ではなかった。**うち **7 件が要対応**（XLS-34 `dataType` ＝ `null` ／ XLS-35 `identifier` ＝ `null` ／ XLS-36 ブロッククラスと `dataType` の不整合 ／ XLS-37 `TestDataContainer.name` ＝ `null` ／ XLS-38 コレクション・Map の `null` と要素 `null` ／ XLS-40 カラム名・フィールド名の重複 ／ XLS-41 `fields` 件数と行の要素数の不一致）、XLS-39（グループ ID に `[` `]` を含む）は対応不要。**足した観点（辺①が本体経由で記法に無い形を持ち込む経路）は新たな検出無し**（XLS-08 以外に無い）。**7 件の修正はいずれも未着手**（集計は 全 44 → 52 件・要対応 16 → 23 ／ 対応不要 27 → 28）
...
- [ ] **§6-C（中間モデル）**: 各具象ブロックの生成時に、自分の系統に属さない `DataType` を拒否する（`issues.md` XLS-36）。**RED まで完了**（`TestDataBlockTest` にテスト 3 件。`wip` コミット済み）。実装は `TableDataBlock` ／ `FileDataBlock` ／ `MessageDataBlock` の `super(...)` 引数位置で検査する形を想定（`ListMapBlock` は `LIST_MAP` を直に渡すため対象外）。**`dataType` ＝ `null` は素通しにして §6-A のメッセージを保つこと**
...
- [ ] **§6-G（中間モデル）**: `fields` の件数と `rows` 各行の要素数の不一致を生成時に拒否する（`issues.md` XLS-41）
- [ ] **最後に 1 回だけ**、課題 ID 単位で要対応／対応不要の実数を確定する（①②の反映と中間モデル点検の後）。**2026-08-18 に中間の最新化を 1 回入れた（`3f38cca`。全 44 件・要対応 15 ／ 対応不要 28 ／ 本作業の対象外 1）**——集計記述が `7200b0f` を反映しておらず、`steering.md:40` が XLS-08・XLS-27〜33 を範囲外に読ませていたため。**この確定ステップ自体は未了**（中間モデル点検は 2026-08-19 に完了したが、未完 8 件の実装が残っているため）
...
- `issues.md` の全 44 件に「NTF 仕様としての判定」欄があり、**要対応 15 件／対応不要 28 件／本作業の対象外 1 件**が出典つきで書かれている（2026-08-18 時点。導出コマンドと出力は `issues.md` 冒頭）。既存の「判断」欄が残っており、両者の違いが説明されている
