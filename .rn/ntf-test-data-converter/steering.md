...
- **JaCoCo の再計測はしない**（ユーザー指示・2026-08-21）。**ただし #48（カバレッジ基準の適用）は指示書が計測を求めるため例外で、計測結果は `checks/step4-2-report.md` §6 と同じく `checks/task-48.md` に置き、`coverage/coverage-report.md` は書き換えていない**（同書が引く行番号を自己無効化させないため。2026-08-31）。#26 の計測は `da66425` 固定であり、全数値の出典は `coverage/coverage-report.md` §0 が記録した 1 回の実行（`jacoco.csv` md5 `d28e374e9027ade63d7919f7a7b5826e`）である。流し直すと行番号と数値が動き、`coverage-report.md` が引用する行番号がすべて自己無効化する
...
- [x] 本体に解釈させたボディ行を取り出す口を `TestCoreReaderAdapter` へ足す（インタープリタ列は
      本体パーサと同じもの。生行を原文で取る既存の口は変えない）
- [x] `XlsFormatReader` のテーブル・`LIST_MAP` の写しで、マーカー除外後のカラム名が 0 件のときだけ
      マーカーカラムの版面を使うようにし、`rowCount` の行落としを外す
- [x] 実 `.xlsx` を入力に、本体 `BasicTestDataParser` を正解としたテストを足す（エントリ数・並び）
- [x] 実データカラムを持つブロックのマーカーが従来どおり消えることを固定する（非回帰）
- [x] 旧仕様を期待していた既存テストを新仕様へ直し、全件を `checks/task-50.md` に列挙する
...
- [x] `YamlFormatReader` のカラム名決定を「非マーカーが 1 件も無ければマーカーカラムを採る」へ改める
- [x] マーカーカラムだけのエントリの行値を、器ではなく同じ Map の原文から復元する
      （器はマーカーカラムを持たないため）
- [x] `YamlTestDataParser` を正解としたテストを足す（エントリ数・並び）
- [x] 実データカラムを持つエントリのマーカーが従来どおり消えることを固定する（非回帰）
- [x] 直した既存テストを全件 `checks/task-51.md` に列挙する
...
**Purpose**: `5783b35` の `implementation/testdata_examples.rst:2231`「全フィールドが空文字のレコードは、いずれか1つのフィールドに ``""`` と記述する。全セルを空にした行は読み飛ばされ、レコードにならないためである。」記載例（同 `:2237`-`:2260`）を往復させるとレコードが 1 件消える。

**件数の訂正（#33 の実測・2026-08-27）**: 指示書と旧 Purpose の「原本 3 件・往復後 2 件」は誤りで、**原本 2 件・是正前の往復後 1 件**である。当該 list-table は 6 行から成り、内訳は識別行（`:2237`-`:2240`）／ディレクティブ行（`:2241`-`:2244`）／名前行（`:2245`-`:2248`）／型行（`:2249`-`:2252`）／データ行 2 件（`:2253`-`:2256`・`:2257`-`:2260`）である。**症状（往復で 1 件消える）の記述そのものは正しい。**記載例そのままの実 `.xlsx` を起点にした実測は `XlsEmptyEntryTest#roundTripsDocumentedVariableFileExampleThroughXls`。
...
- [x] `XlsFormatReader#isEmptyCell` が `""`・`””` を空セル扱いするのをやめる。本体と同じく、生セルが `null` か空文字のときだけ空とする（テーブル・`LIST_MAP` の両経路に効く）
- [x] ファイル・メッセージのデータ行を Excel 形式へ書き出すとき、全要素が空文字になる行は先頭要素を `""` と書く
- [x] テーブル・`LIST_MAP` のエントリを Excel 形式へ書き出すとき、全要素が空文字になるエントリは**各セルへ `""` と書く**
- [x] 直す前に落ちて直したあとに通るテストを用意する
...
- [x] 母集合の各行・各記載例について、**XLS→XLS・XLS→YAML→XLS・YAML→YAML・YAML→XLS→YAML の 4 経路**で、テスティングフレームワークが解釈したあとの値が往復前と一致することを、実ファイル起点のテストで押さえる
- [x] 一致しないものは `@Ignore` ＋ 印つきの理由で記録する（直さない）。**全要素が空文字のエントリが XLS→YAML で落ちる件の理由文は #33 の「`@Ignore` の理由文の訂正」に従う**（「YAML 形式では表せない」と書かない）
- [x] 表の行ごと・記載例ごとに 4 経路それぞれの合否を報告に書く
- [x] #32 で新設した `XlsNotationSymmetryTest`（8 件）と **#33 で新設した `XlsEmptyEntryTest`（12 件）** を `coverage/inventory.md` の軸要素対応表へ載せるかを、本タスクの母集合と重ね合わせて判断し、載せる場合は載せる（ユーザー指示・2026-08-27。#33 の 12 件は `checks/task-33.md`「台帳の更新」で同じ理由により未登録）
- [x] 期待値をわざと崩すと落ちることを 1 度確認する

**#37 の実測（2026-08-28）**: 母集合は表 12 行（改行文字の行を CR／LF の 2 形に分けて **13 件**）＋ 記載例 6 小節（「空文字・改行」の小節は YAML 側の LF 例と全フィールド空文字の例を分けて **7 件**）。**20 件 × 4 経路 ＝ 80 経路すべて合格し、`@Ignore` は 0 件**。担保は新設の `SpecialNotationRoundTripTest`（20 件）。**「アップロードファイルを指定する」の小節は母集合に含めていない**（対象が `${attach:...}` で特殊記法の表に無いため。報告に開示）。**新設 3 クラスは軸要素対応表へ載せない**と判断した（理由は `inventory.md` §4.5）。
...
- [x] 【§8-1】テーブル・`LIST_MAP` とも、**マーカーカラム除外後のカラム名が 0 件のブロックは行を持たせない**（値を見て落とす判定は入れない）
- [x] 【§8-2】(e) 3〜5 の**期待値は変えない**。assert メッセージ・Javadoc の「全要素が空のエントリになるため読み飛ばされる」「XLS-08」を「カラム名が 0 件のため行を持たない」の言葉で書き直す
- [x] 【§8-3】本体が読む件数と一致することを実 `.xlsx` 起点で押さえるテストを足す。**他のカラムを持つブロック**で組む（`[no]`,`id`,`name`／`1,U0001,yamada`・`(空),"",""`・`3,(空),(空)`）
- [x] 【§8-4】台帳エントリに「XLS-08（空エントリ判定をマーカー除外の後に行う）の仕組みを上書きした。結果が変わるのは他のカラムを持つブロックだけで、カラム名 0 件のブロックの結果（行 0 件）は変わらない」と明記する
...
- [x] 先に落ちるテストを書く（(i) 警告1件・(ii) 出力に無い・(iii) 本体が読む結果が一致）
- [x] (i) で落ちることを確認する
- [x] `readHeaders` の並びから再出現を検出して `LOGGER.warning` を出す（`LIST_MAP`・`MESSAGE` は対象外）
- [x] 期待値をわざと崩すと落ちることを1度確認する
...
- [x] 赤 4 件の期待値を解説書に合わせて直す
- [x] yaml 第2回 2-1〜2-5 の 5 項目を押さえるテストを足す（2-2 の電文 `records:` は 1 つ／2-3 `fw_header:` のキー／2-5 2文字 `\r`）
- [x] 期待値をわざと崩すと落ちることを1度確認する
...
- [x] `.rst` パス・`nablarch-document`・「解説書」「出典」・節見出し・逐語引用を取り除く
- [x] 他リポジトリのソースを `path:line` で指す 11 行はクラス名だけ残す
- [x] テストの動作・期待値は変えない（コメント・Javadoc・assert メッセージの文字列だけ）
- [x] 単独のコミットにする
...
- [x] HEAD（`a5f006c`）で計測し、`jacoco.xml` から未達行・未達分岐を機械抽出する
- [x] 未達全件を (a)(b)(c) に分類し、根拠を書く
- [x] (a) のテストを追加し、1 件ずつ変異で落ちることを実測する
- [x] (b) を削除する（削除前に `git grep -n` で呼び出し側を全走査）
- [x] 再測定し、残る未達が (c) の一覧と完全一致することを確認する
- [x] `checks/task-48.md` に分類・変異・grep・測定ログを記録する
...
**#49 は承認された。**指示書 `ntf-step4-09-converter-coverage.md` §5 の第2ラウンドはこれで完了で、
追加タスクは無い。承認したのは C6 の `else` 節（`throw` 1 文）と (c) 全箇所の理由コメントで、
正は `checks/task-49.md`（差分・コメント全件の逐語・再測定の対応表）。

**カバレッジ基準（未達 0）の適用はこれで終わった。**残る未達 30 行／8 分岐はすべて承認済みの到達不能
箇所であり、全箇所が「なぜ到達不能か」「なぜ残しているか」の両方をコメントで述べている。
