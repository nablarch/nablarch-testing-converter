...
- [ ] #32 で新設した `XlsNotationSymmetryTest`（8 件）と **#33 で新設した `XlsEmptyEntryTest`（12 件）** を `coverage/inventory.md` の軸要素対応表へ載せるかを、本タスクの母集合と重ね合わせて判断し、載せる場合は載せる（ユーザー指示・2026-08-27。#33 の 12 件は `checks/task-33.md`「台帳の更新」で同じ理由により未登録）
