...
- 本作業で見つかった現状挙動の課題が課題一覧へ記録されている。**#25.5 で要対応と判定した 5 件は修正され、それ以外は修正されずに記録のみである**（ユーザー確定・2026-08-14 の方針変更に合わせて改訂。改訂前は「修正されずに」が全件に掛かっていた）
...
- 1 task = 1 commit
- （フェーズ1）実装の変更は一切しない。package/import の機械的調整と pom 設定のみ許可。**フェーズ2 の #25.5 だけがこの例外で、要対応 5 件の修正に限り src/main を変更する**（ユーザー確定・2026-08-14）
- 本体・yaml には書き込まない
...
- **本作業で見つかった不具合は、要対応と判定した 5 件（YML-02・YML-12・XLS-16・XLS-06・YML-08）に限り修正する。それ以外は従来どおり記録のみ**（ユーザー確定・2026-08-14）。記録先は `.rn/ntf-test-data-converter/coverage/issues.md`。修正対象の判定根拠と手順は Decisions「不具合修正の対象と手順（#25.5）」
...
5 件に限り修正する**。それ以外は従来どおり記録のみである。Rules の該当 2 行と Acceptance criteria の
2 行はこの決定に合わせて改訂済み。
...
**YML-12 で直すのは 4 形のうち 1 つ目だけ。** `issues.md` YML-12 の「あるべき姿」は 4 形に分かれており、
...
認めておらず、`[]` を書いても読み戻せない。**この 3 形は今回の対象外とし、記録のみのまま残す。**
...
**Purpose**: 要対応と判定した 5 件を TDD で修正し、不具合の挙動を緑のアサートで固定している状態を解消する。判定と出典は Decisions「不具合修正の対象と手順（#25.5）」。
...
- 5 件それぞれについて、仕様どおりの期待値のテストが存在し、**修正前に赤になったこと（失敗メッセージ）が記録されている**
- 5 件それぞれについて、現状挙動を固定していた既存テストが削除されている（同じ挙動を主張するテストが 2 本残っていない）
...
- `issues.md` の全 35 件に「NTF 仕様としての判定」欄があり、要対応 6 件／対応不要 29 件が出典つきで書かれている。既存の「判断」欄が残っており、両者の違いが説明されている
- YML-08 の「未確認」が実行結果で埋まっている
- `mvn clean test -Djacoco.skip=true` が全テスト PASS する（`@Ignore` は Skipped として現れてよい）
- `src/main` の変更が 5 件の修正に必要な範囲に限られており、変更したファイル名・課題ID・変更理由が記録されている
