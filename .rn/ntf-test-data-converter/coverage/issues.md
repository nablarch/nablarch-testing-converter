...
食い違う 12 件（XLS-05・XLS-10・XLS-12・XLS-14・XLS-21・YML-01・YML-04・YML-05・YML-06・YML-07・
...
課題は全部で **54 件**（課題 ID 単位）。内訳は **要対応 24 件**／**対応不要 27 件**／**保留 2 件**／
**本作業の対象外 1 件**（XLS-26）である。数え直した日は 2026-08-19（**XLS-41 を「多い側」に限定して
要対応へ戻し、少ない側を XLS-42、ディレクティブ Map の `null` を XLS-43 として切り出したあとの値**）。
導出コマンドと出力は下記「集計は課題 ID 単位で数えること」の節にあり、
`24 + 27 + 2 + 1 = 54` が機械で確かめられる。
...
> ディレクティブ Map の `null` を XLS-43（要対応）として切り出したため、
> 現在は 全 54 件・要対応 24 ／ 対応不要 27 ／ 保留 2 ／ 本作業の対象外 1 である**（冒頭の内訳を参照）。
...
上の出力は 2026-08-19 に実行して得たものである（**XLS-42・XLS-43 を切り出したあとの値**）。
`24 + 27 + 2 + 1 = 54` で、1 つ目（`###` 見出しの数）とも 2 つ目（判定欄の行数）とも一致する。
