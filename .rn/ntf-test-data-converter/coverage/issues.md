...
`COV-nn` は「**確かめていない**」と言っている。**問うていることが違うため、件数も判定欄も別に扱う**（§7 の冒頭）。
...
**この節は、本節より前に並ぶ各項とは別種の記録である。** 本節より前の `XLS-nn` ／ `YML-nn`
（`## #19`〜`## #25.5` の下にある各項）は**現状挙動の課題**——実行して記録した挙動のうち仕様として
妥当でないもの——であるのに対し、ここに載せる `COV-nn` は**担保の穴**——挙動が仕様どおりかどうかを
確かめていない未到達分岐——である。挙動が不正だと言っているのではなく、**確かめていない**と言っている。
...
- 備考: `inventory.md` §3.1-2 が「軸要素の外にある開示すべき担保の穴」として既に開示している。
