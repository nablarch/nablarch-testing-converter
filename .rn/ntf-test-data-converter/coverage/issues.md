...
**さらに 2026-08-24 に XLS-45 の判定が「対応不要」から「要対応」へ変わり、#31 で実施した。**
NTF 仕様として可変長ファイルでは `length` を書けないことが確定したためである（ユーザー確定。
YAML 記法は未リリースであり、記法側を変更してよい）。**#31 で可変長のフィールド定義が `length` を
持てないようにし、辺③で値が黙って落ちる経路そのものを断った。**
**「#25.5 の時点の要対応（修正済み）」「#27 で立てて #29 で実施した要対応」「#27 で立てて #31 で
実施した要対応」は下の内訳で読み分けること。**
...
食い違う 13 件（XLS-05・XLS-10・XLS-12・XLS-14・XLS-21・YML-01・YML-04・YML-05・YML-06・
...
**XLS-45 は 2026-08-24 に同じく「対応不要」から「要対応」へ変えたため外した** ——
可変長ファイルで `length` を書けないことが NTF 仕様として確定し、
converter 側で閉じられるようになったためである。XLS-45 の節の【判定の訂正】を参照）

課題は全部で **57 件**（課題 ID 単位）。内訳は **要対応 27 件**／**対応不要 27 件**／**保留 2 件**／
**本作業の対象外 1 件**（XLS-26）である。数え直した日は 2026-08-24
（**2026-08-21 の 26 ／ 28 から、XLS-45 が「対応不要」から「要対応」へ移って 1 件ずつ動いたあとの値**。
総数 57 は変わっていない）。導出コマンドと出力は下記「集計は課題 ID 単位で数えること」の節にあり、
`27 + 27 + 2 + 1 = 57` が機械で確かめられる。

**要対応 27 件の内訳（2026-08-24 時点）。「#25.5 で修正済み」「#27 で起票し #29 で実施」
「#27 で起票し #31 で実施」を分けて書く。**
...
- **#27 で起票し、#31 で実施した 1 件** —— **XLS-45**（辺③ が可変長ファイルの `length` を黙って落とす）。
  #27 の起票時は判定が **対応不要**だったが、**2026-08-24 に「NTF 仕様として、可変長ファイルでは
  `length` を書けない」ことが確定した**（ユーザー確定。YAML 記法は未リリースであり、記法側を
  変更してよい）ため **要対応**へ移り、**#31 で中間モデルの生成時に拒否する形で閉じた**
  （`ModelPreconditions#requireNoLengths`）。**辺③④のライタには番人も WARN も足していない** ——
  可変長が `length` を持てなくなれば、値が落ちる経路そのものに到達しないためである。
  **したがって要対応 27 件はすべて実施済みである。**
...
`length` を黙って落とす）を起票して 28 件になり、2026-08-24 に同じ XLS-45 が要対応へ移って
27 件に戻った**（可変長で `length` を書けないことが NTF 仕様として確定したため。XLS-45 の
【判定の訂正】）。
...
上の出力は 2026-08-24 に実行して得たものである（**#31 で XLS-45 が要対応へ移ったあとの値**）。
`27 + 27 + 2 + 1 = 57` で、1 つ目（`###` 見出しの数）とも 2 つ目（判定欄の行数）とも一致する。
...
>
> **HEAD はさらに進んでいる（2026-08-24 実測）。** 同ブランチの HEAD は `3132688` である。
> #31 で引いた `:1076`「固定長との違いは、可変長ファイルの場合はフィールド長行を記載しない点のみである。」は
> `30a8271` と HEAD の両方で同じ行番号・同じ本文であることを確かめた
> （`git -C ~/work/nablarch/nablarch-document grep -n '可変長ファイルの場合はフィールド長行を記載しない' -- …/testdata_notation.rst`）。
> **本書の行番号の基準は `30a8271` のまま変えていない。**
...
- NTF 仕様としての判定: **要対応**（**2026-08-18・ユーザー判断で「対応不要」から変更**。修正済み）。
  ...
  - **解説書は逆の順序で明文化された。ただし往復の結果としては食い違わない（2026-08-27・ユーザー確定）。**
  ...
  - **一時「現行の解説書と食い違う」と追記したが、これは誤りであった（2026-08-27・ユーザー確認により訂正）。**
    `notation.rst:1500`（本体の読み）と、同じピン `5783b35` の
    `ja/development_tools/testing_framework/tools/testdata_converter.rst:63`
    「マーカーカラムだけに値があるエントリも、往復すると消える。……全要素が空のエントリを
    記述する記法が無いため、変換先の形式で表せない。」は**両立する**。
    違うのは**中間モデルの形**（converter が読みで落とすか、書きで落ちるか）だけであり、
    **利用者から見える往復の結果は解説書のとおり**である。
  - **したがって XLS-08 は現状のままでよい。#33（2-2）でも直さない**（ユーザー確定・2026-08-27）。
  - **【2026-08-29・#41】XLS-08 の仕組みを上書きした。**「空エントリ判定をマーカーカラム除外の後に行う」
    という機構は取り除いた（`XlsFormatReader#dropEmptyEntries`・`#isEmptyEntry`・`#isEmptyCell` を削除）。
    代わりに置いたのは**カラム名の数だけで決める判定**である —— マーカーカラムを除いたカラム名が 0 件の
    ブロックはデータ行を持たない（`XlsFormatReader#rowCount`）。**値は見ない。**
    - **結果が変わるのは他のカラムを持つブロックだけである。** マーカーカラムだけに値があるエントリが
      残るようになった（`XlsMarkerOnlyEntryTest`。本体が読むのと同じ 3 件）。
    - **カラム名が 0 件のブロックの結果（行 0 件）は変わらない。** 上表の「修正後」がそのまま現在の値である
      （`XlsFormatReaderRealFileTest#dropsMarkerOnlyRowsAsEmptyEntriesInRealBook` ／
      `#dropsMarkerOnlyRowsAsEmptyEntriesInListMapInRealBook` ／
      `XlsReferenceFixtureTest#readsExcelSavedWorkbookIntoIntermediateModel` の `block[3]`。**期待値を変えていない**）。
    - **上書きした理由**は 2 つある。(1) 値で落とす判定は、本体には届く行（全セルが `null` 記法の行・
      全セルが空文字記法の行）まで消してしまう。本体にセルを解釈させる配線へ変えた（#40）ことで、
      解釈後の値で判定することになり、この取りこぼしが顕在化した。
      (2) 解説書 `tools/testdata_converter.rst:63`（`a6da1f6`）が 2 つを別々に定めた ——
...
| 入力 | 中間モデルへ入る結果 | 担保テスト |
|---|---|---|
| `SETUP_TABLEX=T`（`SETUP_TABLE` の綴り誤り） | **#34 前**: `dataType` ＝ `SETUP_TABLE_DATA`、`groupId` ＝ `"X"`（角括弧なし）、`identifier` ＝ `"T"` ／ **#34 後**: ブロックが 1 つも作られない | `XlsFormatReaderInvalidInputTest#dropsMarkerWhoseGroupIdIsNotBracketedInRealBook`（**#34 で改称**。旧 `#readsSuffixAfterKnownDataTypeNameAsGroupIdInRealBook`） |

- 原因: **本リポジトリの `src/main/java/nablarch/test/core/reader/TestCoreReaderAdapter.java`**（`nablarch-testing` 側ではない）の
  `markerGroupId`（L282-286）がデータタイプ名の直後から `=` までを無条件に
  グループ ID として切り出す。正しい記法は `SETUP_TABLE[g1]=T` のように角括弧付きだが、角括弧の有無は
  検証されない。
- 影響（**#34 前**）: 変換後の YAML に作成者が意図しない `group_id: "X"` が出る。値そのものは失われず、
  Excel へ書き戻せば元の `SETUP_TABLEX=T` に戻る（往復は安定する）。
- **観測できる挙動が #34（2-3）で変わった（2026-08-27）。** 中間モデルのグループ ID を生値にした結果、
  `markerGroupId` は `X` を返し、上流 API へ渡すときに `[X]` へ整形される
  （`GroupIdNotation#format`）。版面の `SETUP_TABLEX=` とは前方一致しないため、
  **このブロックは黙って消える**（変換後の Excel・YAML のどちらにも現れない）。
  #34 前は生値をそのまま上流へ渡していたため一致していた。
  **角括弧の無いグループ ID は解説書に無い書き方であり、追いかけない**
  （ユーザー確定・2026-08-27。`TABLE[]=x` と同じ扱い）。判定欄（対応不要）は変えない。
...
- **【2026-08-31・#53】拒否の範囲を「カラム名 0 件で行を 1 件でも持つ形」へ広げた。**
  従来は「セルを 1 つ以上持つ行」だけを拒否し、「カラム名 0 件かつ<b>セルを 1 つも持たない行</b>」は
  `notation:1550` のマーカーカラム除外の結果として**辺①・辺②が仕様適合入力から実際に作る形**であるとして
  通していた（XLS-08 ／ YML-04）。**その 2 辺がマーカーカラムの名前と値を保つようになり
  （XLS-08 の【2026-08-31・#50】）、この形はどちらの読みからも作られなくなった。**
  どちらの記法にも書けない形でもある —— YAML へ書くと `- {}` になり、読み戻すと行として存在しないものとして
  取り除かれる。担保テストは `TableDataBlockTest#カラムなしで行を持つブロックは生成できない`。
  あわせて `YamlFormatWriter#emitMapRows` のカラム 0 件分岐（`- {}` を書く経路）を削除した。
- 「カラム名 0 件かつ行 0 件」（XLS-27 の 0 件テーブル）は従来どおり通す
  （`TableDataBlockTest#カラムなし行なしのブロックは生成できる`）。辺②が `rows: []` から実際に作る形である。
...
- 原因: **帰属は yaml 側と converter の両方である。**
  ...
  - **テーブル経路は `nonMarkerColumns` を通らない。** `YamlFormatReader#addTableBlocks` は
    ...
    **（2026-08-31・#51）`addTableBlocks` はマーカーカラムだけのエントリを扱うために Map エントリを
    見るようになったが、カラム名を決めるのは器のままであり、この節の主張は変わらない。**
...
- 影響: 手書きの YAML で「その行にだけ意味のあるカラム」を後の行に足す書き方（NULL 許容カラムを 1 行だけ書く等）は
  ...
  ブロックの中身が丸ごと消える。
- **XLS-08 との関係（2026-08-31・#51 で更新）**: カラム名の行がマーカーカラム（`[no]`）だけのエントリは、
  **マーカーカラムの名前と各行の値を保つ**ようになった（担保テスト
  `YamlFormatReaderInvalidInputTest#keepsMarkerOnlyTableColumnsAndValues` ／
  `YamlMarkerOnlyBlockTest`）。辺①の **XLS-08** と同じ扱いであり、2 辺は対称である。
  従来ここは「カラム 0 件・値を持たない行」になっており、それが XLS-08 と同型の現れ方であった。
  マーカー除外そのものは意図した仕様（steering #15）であり、**実データカラムを 1 つでも持つエントリでは
  従来どおりマーカーカラムが落ちる**。本課題は「先頭行だけでカラムが決まる」ことのほうを指す。
...
- **【追記・2026-08-19・§6-J-3（`b762438`）】番人を書き出し側から中間モデルへ移した。**
  ...
  生成時に拒否する）**。
  - **【追記・2026-08-24・#31】可変長ファイルには逆向きの番人が付いた。** 上の「拒否しない」は
    **`null` を拒否しない**という意味であり、変わっていない。#31 で足したのは
    `ModelPreconditions#requireNoLengths` で、可変長系のデータ種別のとき `length` が
    **`null` でない**フィールド定義を拒否する（`issues.md` **XLS-45**）。
    `FileDataBlock` のコンストラクタは固定長系で `#requireLengths`、可変長系で `#requireNoLengths` を呼ぶ。
    **XLS-30 の番人・例外メッセージ・境界（弾くのは `null` だけで空文字は弾かない）は変えていない。**
...
- **#34（2-3）で解消した（2026-08-28・プローブ実測）。** 中間モデルの `groupId` を生値にした結果、
  辺④の `rawGroup`（推測剥がし）と辺②の `formatGroup`（角括弧付与）がどちらも消え、
  **辺③と辺④が同じ文字列を指すようになった。** `groupId` ＝ `"[a]x[b]"` の中間モデルを渡した実測は
  —— 辺③が `SETUP_TABLE[[a]x[b]]=T` を書き、辺①で読み戻すと `"[a]x[b]"` ／
  辺④が `group_id: "[a]x[b]"` を書き、辺②で読み戻すと `"[a]x[b]"`。
  **4 経路とも同じ値で往復し、Excel と YAML が別のグループを指す食い違いは起きない。**
  （プローブは実行後に削除した。恒久テストは足していない —— #34 の完了条件が求めるグループ ID 4 種
  （省略・単一・複数・送信系）に区切り文字入りは含まれないため。）
- NTF 仕様としての判定: **保留**（明文が無く、converter 側では決められない）。→ 直後の申し送りを参照。
  **判定そのものは変えていない**（グループ ID に使える文字を定める明文が無いことは今も変わらないため）。
  食い違いという症状だけが消えた。
...
#25.5 で修正した課題に限られていること」を求めているためである。**2 件とも後続の独立タスクで実施した
（XLS-44 は #29、XLS-45 は #31）。****XLS-44 は判定が要対応で、
#28 の承認後に立てた独立タスク #29 で実施した**（マージ前に片付ける —— ユーザー確定・2026-08-21。
＝ 冒頭の内訳で「#25.5 で修正済みの 25 件」とは別枠に置いた 1 件）。
**XLS-45 は起票時の判定が対応不要だったが、2026-08-24 に要対応へ変わり #31 で実施した**
（NTF 仕様として可変長ファイルでは `length` を書けないことが確定したため。ユーザー確定。
起票時の判定「Excel 記法に書ける先が無く、明文に反していない」の覆り方は XLS-45 の【判定の訂正】）。
...
### XLS-45 辺③ が可変長ファイルの `length` を黙って落とす（影響度 中・**検出できない**／記法どうしの非対称。**#27 で起票。#31 で修正済み**）

> **【起票時の但し書き —— 2026-08-24 に覆った】** 起票時（#27）は次のように書いていた ——
...
>
> **この但し書きは前提ごと覆った。** 前提は「可変長＋`length` は NTF の YAML 記法として書ける」で
> あったが、**NTF 仕様として可変長ファイルでは `length` を書けないことが確定した**
> （ユーザー確定・2026-08-24。YAML 記法は未リリースであり、記法側を変更してよい）。
> 書けないのだから中間モデルが保持することも正当でなく、**生成時に拒否するのが正しい**。
> 「拒否すると辺②が読めなくなる」は<b>そのとおりで、それが意図した振る舞いである</b> ——
> 可変長ファイルに `length` を書いた YAML は仕様外の入力であり、読み込んだ時点で落ちてよい。
> **#31 でそのとおりに実装した**（下記「converter 側の対応」）。

**本課題の当初の主張は、辺③（中間モデル → Excel）で値が黙って消えることである。**
**#31 は消える経路そのものを断った** —— 可変長のフィールド定義が `length` を持てなくなったため、
辺③が落とす値がそもそも存在しない。
...
- NTF 仕様としての判定: **要対応**（修正済み・#31）。**2026-08-24 に「対応不要」から変えた。**
  【判定の訂正】旧判定は「**対応不要**（**明文に反していない**）。Excel 記法は可変長にフィールド長行を
  持たないと `notation:1076`・`:883` が定めており、辺③が `length` を書かないのは明文どおりである。
  書ける先が無いため converter 側では閉じられない。上の『判断』（仕様として不適切）と食い違う。
  YML-10・YML-14 と同じく、指摘としては正しいが直す場所が converter ではない型である」だった。
  **旧判定は「可変長＋`length` を書ける入力を、converter が受け取ってしまう」ことを所与にしていた。**
  その所与が覆った —— **NTF 仕様として、可変長ファイルでは `length` を書けない**
  （ユーザー確定・2026-08-24。YAML 記法は未リリースであり、記法側を変更してよい）。
  禁止されている以上、可変長＋`length` を保持する中間モデルは<b>どの入力からも作られてはならない状態</b>で
  あり、converter 側で閉じられる —— 閉じる場所は辺③の書き出しではなく<b>中間モデルの生成時</b>である
  （`steering.md` Decisions「不正値は書き出し側でなく中間モデルの生成時に拒否する」）。
  **「判断」欄（仕様として不適切）との食い違いは解消した。**
- **converter 側の対応（#31）**: `ModelPreconditions#requireNoLengths` を足し、`FileDataBlock` の
  コンストラクタが<b>可変長系のデータ種別のとき</b>これを呼ぶ（固定長系は従来どおり
  `#requireLengths`）。**弾くのは `null` 以外のすべてで、`"-"` も空文字も弾く**（固定長側とは境界が逆）。
  **辺③④のライタには番人も WARN も足していない** —— 可変長が `length` を持てなくなれば、
  `XlsFormatWriter` が黙って落とす経路も `YamlFormatWriter` の非対称も到達しないためである。
  **`YamlTestDataValidator` も改修していない** —— スキーマをクラスパスの
  `/nablarch/test/ntf-testdata-yaml-schema.json` から読むだけであり、スキーマ側の対応が入れば自動で追随する
  （スキーマ対応が入るまでスキーマ検証が通ることは織り込み済みである。落ちる段が中間モデルの生成時から
  スキーマ検証へ前へ動くだけで、落ちること自体は変わらない）。
  **本体（`nablarch-testing`）も直していない** —— 可変長で `length` が来なくなれば
  `nablarch-testing-yaml` の `YamlFileBuilder` にある `messaging || hasLength` の分岐が偽になり、
  本節が上で記した `"-"` のトリム経路（`DataFileFragment#setLengths` →
  `#addValue` の `removeLineSeparatorWithTrim`）に入らない。**放置ではなく到達しなくなる。**
- 担保テスト: `FileDataBlockTest#可変長ファイルでフィールド長を持つフィールド定義は保持できない` ／
  `FileDataBlockTest#フィールド長を持つフィールド定義は可変長系のデータ種別すべてで拒否される`
  （可変長系 2 種 × `"10"` ／ `"-"` ／ 空文字の 6 通り）／
  `YamlFormatReaderInvalidInputTest#rejectsVariableFileFieldWithLengthFromRealYaml`
  （**辺②の実ファイル経路**。`loadRawMap` 差し替えの in-memory 経路は担保に数えない）。
  従来からの記録は `XlsFormatWriterTest#writesVariableFileWithoutLengthRow`（辺③で長さ行が出ないこと）／
  `YamlFormatWriterTest#serializeFile_variableOmitsDirectivesAndRecordTypeAndLength`（辺④で
  `length` キーが出ないこと）である。**「可変長で `length` を持つ中間モデルを辺③へ渡すと値が落ちる」ことを
  主張するテストは無い** —— そのような中間モデルを作れなくなったためである
  （`axis-matrix.md` 辺③ 軸C の C-21(省略) は #31 で ❌ から ✅ へ動いた。その行の理由欄）。
- **解説書担当・スキーマ担当宛の申し送りは、問いから報告へ変わった**（直後の節）。
  起票時（2026-08-21）は「可変長の `length` を無視／禁止／任意のどれにするか決めてほしい」という
  問いだったが、**禁止で確定した**（ユーザー確定・2026-08-24）。converter 側は確定に合わせて実装済みで
  あり、残るのは本体スキーマと解説書の対応である。**下記「申し送りの束」からは外した**（束は 4 件）。

#### 可変長フィールドの `length` は禁止で確定（XLS-45。2026-08-24）

**これは問いではない。決着した事項の記録である。** 起票時（2026-08-21）は解説書担当・本体スキーマ担当宛に
「可変長フィールドの `length` を NTF としてどう扱うのか（無視／禁止／任意）を明文化してほしい」という
申し送りを立てていた。**2026-08-24 に禁止で確定した**（ユーザー確定。YAML 記法は未リリースであり、
記法側を変更してよい）。**スキーマと解説書の対応は別途依頼済みである。**
したがって本件は下記「申し送りの束」から外してある（束は 4 件）。

**確定した内容 —— 可変長ファイルのフィールド定義に `length` を書けない。**

- **converter 側は #31 で実装済み。** 可変長系のデータ種別の `FileDataBlock` は、`length` を持つ
  フィールド定義を保持できない（生成時に `IllegalArgumentException`）。`"-"` も空文字も弾く。
- **本体スキーマ側は未対応**（`nablarch-testing-yaml` の
  `src/main/resources/nablarch/test/ntf-testdata-yaml-schema.json`）。
  `$defs.field_def.properties.length` の description は可変長について「可変長ファイルでは不要（省略可）。」と
  だけ書いており、**記載を禁じていない**（`anyOf` は `integer`（`minimum: 0`）か `^([0-9]+|-)$` の
  文字列の 2 択で、可変長を除く条件は無い）。**スキーマ側の対応が入るまで、可変長＋`length` の YAML は
  スキーマ検証を通り、converter の中間モデル生成時に落ちる。**対応が入れば落ちる段がスキーマ検証へ
  前に動くだけで、落ちること自体は変わらない。`YamlTestDataValidator` はスキーマをクラスパスから
  読むだけなので、converter 側の改修は要らない。
- **解説書側は元から禁止と読める。** `testdata_notation.rst`（`30a8271` 時点）は
  `:1076`「固定長との違いは、可変長ファイルの場合はフィールド長行を記載しない点のみである。」・
  `:883`「…可変長ファイルでは、フィールド名称・データ型の2リストが同サイズで必須であり、
  フィールド長は不要である。」と書く。**Excel 記法には可変長のフィールド長行が無い。**
- **禁止にする理由**（決定の根拠。ユーザー確定・2026-08-24）: 可変長では数値の `length` は NTF 実行時に
  使われない（本体 `nablarch/test/core/file/VariableLengthFileFragment.java` の
  `createFieldDefinition` は `name` ／ `position` ／ `encoding` だけを設定し、`lengths` を一度も読まない）。
  効いてしまうのは `length: "-"` だけで、これは
  `nablarch/test/core/file/DataFileFragment.java` の `setLengths` が `"-"` を
  `isOndemandCalcFieldSizeList` に立て、`addValue` が当該フィールドの値から改行コードと前後空白を
  除去することによる。**これは長さの指定ではなく値の整形であり、フィールド長の枠に相乗りしている。**
  可変長でこれが効くのは相乗りの結果であって、追認すべき仕様ではない。

**行番号は `30a8271` 時点の値である**（本書全体と同じ基準）。
...
**XLS-45 は 2026-08-24 に束から外した。**「可変長フィールドの `length` をどう扱うか」は問いでなくなり
（禁止で確定・ユーザー確定 2026-08-24）、スキーマと解説書の対応は別途依頼済みだからである。
経緯は「可変長フィールドの `length` は禁止で確定（XLS-45。2026-08-24）」の節にある。
...
- 処理: テスト未追加。残課題として記録
- **#34（2-3）で本項は消えた（2026-08-28）。** 未到達箇所とした `YamlFormatWriter#rawGroup` そのものを
  削除したためである（中間モデルの `groupId` が生値になり、書き出し側は加工しない）。
  JaCoCo は再計測していない（Rules「JaCoCo の再計測はしない」）
