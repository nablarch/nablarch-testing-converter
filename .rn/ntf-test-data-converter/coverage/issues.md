...
- **解説書担当・スキーマ担当宛の申し送りを立てた**（ユーザー確定・2026-08-21。直後の節）。
  ...
  **ただし単独では出さない。** converter 完了後に U-1・XLS-27・XLS-40・XLS-42 とまとめて出す束に加える
  （下記「申し送りの束」）。
...
| # | 申し送り | 宛先 | 本文の場所 |
|---|---|---|---|
| 1 | **U-1** | （未確認） | **本リポジトリの文書に `U-1` という ID は存在しない**（`grep -rn 'U-1' .rn/ntf-test-data-converter/` → 0 件。`nablarch-testing/.rn` にも無い。2026-08-21 実測）。**ユーザーの手元にある ID と思われる。渡す前に所在を確認すること。** |
| 2 | **XLS-27** 0 件テーブル | 対象 PJ・解説書担当 | 「0 件テーブルの申し送り（XLS-27…）」の節 |
...
**束に入れていない申し送りが 1 件ある。** 「グループ ID に使える文字の明文化（XLS-39。解説書担当宛）」である。
**束へ含めるかはユーザー未指定**（2026-08-21 の指示は U-1・XLS-27・XLS-40・XLS-42・XLS-45 の 5 件を挙げている）。
