...
- 根拠: `notation:622`「``LIST_MAP`` のID は完全一致で検索される。**同一の読み込み単位内に
  ...
  **`MESSAGE` にも同じ先着一致が明文で及ぶ** —— `notation:262-270` の収集方式テーブルが
  「**単一**」方式を「データタイプが合致する（Excel 形式ではデータタイプ名の前方一致）データブロックのうち、
  **ID が完全一致する最初の1件を取得**」と定め、該当データタイプに ``LIST_MAP``・``MESSAGE`` を
  明示している（`notation:266-267`）。実装は先着一致（`processed` セット）を採っており、明文どおりである
...
- 備考: 本番の唯一の呼び出し元 `XlsFormatHandler.java:46` は
  ...
  **「本番の呼び出し元が作らない」＝「到達経路が無い」を分類の根拠にはしない** ——
  XLS-21 の【判定の訂正】（2026-08-19）が「**『到達経路が無い』は判定の根拠にしない**（XLS-22 で
