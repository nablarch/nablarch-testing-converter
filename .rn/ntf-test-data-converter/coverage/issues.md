...
食い違う 13 件（XLS-05・XLS-10・XLS-12・XLS-14・XLS-21・YML-01・YML-04・YML-05・YML-06・YML-07・
YML-09・YML-10・YML-14）では、判定欄の中でその旨を明示した。**「判断」欄は 1 件も取り消していないので、両方を読むこと。**
...
- **修正済み 25 件** —— XLS-06・XLS-08・XLS-16・XLS-20・**XLS-21**・XLS-22・**XLS-27**・XLS-28・XLS-29・XLS-30・XLS-31・XLS-32・XLS-33・XLS-34・XLS-35・XLS-36・XLS-37・XLS-38・XLS-40・XLS-41・XLS-43 ／
  ...
  - **XLS-27 は §6-K（`839bf64`）で閉じた。** 0 件テーブルはマーカーカラム 1 列（`[空]`）で
...
> | 番人 | 課題 | 決着 |
> |---|---|---|
> | `fileType` ＝ `null` | XLS-29 | **モデルへ移設**（`FileDataBlock` の生成時。§6-J-1・`7b0b381`） |
> | `records` 空（電文） | YML-12 2 形目 | **モデルへ移設**（`MessageDataBlock` の生成時。§6-J-2・`9e40644`） |
> | `fields` 空 | XLS-22 | **モデルへ移設**（`RecordLayout` の生成時。§6-I・`c31b534`） |
> | `columnNames` 0 件 | XLS-27 | **撤去**。0 件テーブルはマーカーカラム 1 列 `[空]` で書けるため、変換を中止する理由が無い（§6-K・`839bf64`）。あわせて「カラム名 0 件で<b>セルを持つ</b>行」を `ColumnRowDataBlock` の生成時に拒否した（XLS-21） |
...
- NTF 仕様としての判定: **要対応**（**2026-08-18・ユーザー判断で「対応不要」から変更**。修正済み）。
  ...
  - **`nablarch-example-web` 由来の実フィクスチャに、この形が実在する**（`XlsReferenceFixtureTest` の
    ...
    **—— この 2 文は 2026-08-18 時点の記述であり、もう成り立たない（2026-08-19・§6-K）。**
    XLS-27 の辺③番人は撤去され、カラム名 0 件のブロックはマーカーカラム 1 列 `[空]` として
...
- **改修**: `XlsFormatWriter#layoutColumnRow` は `columnNames` が空のとき、マーカーカラム 1 列
  `EMPTY_BLOCK_MARKER_COLUMN` ＝ `[空]` だけのカラム名行を書く。本体パーサが `:1550` により
...
- 機構: `YamlFormatWriter.java:526-535` の `rawGroup` が、先頭が `[` かつ末尾が `]` のときに外側 1 文字ずつを
  外す。逆写像は `YamlFormatReader.java:485-487` の `formatGroup`（`"[" + groupId + "]"`）である。
