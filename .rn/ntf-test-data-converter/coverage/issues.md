...
**要対応 23 件の内訳（修正済み 16 ／ 未完 7。2026-08-19 時点）**

- **修正済み 16 件** —— XLS-06・XLS-08・XLS-16・**XLS-20**・XLS-22・**XLS-28**・XLS-29・XLS-30・**XLS-31**・**XLS-32**・**XLS-33**・**XLS-34** ／
  YML-02・YML-03・YML-08・YML-12（**YML-12 は 4 形すべてが修正済み**）
- **未完 7 件** —— XLS-27・**XLS-35**・**XLS-36**・**XLS-37**・**XLS-38**・**XLS-40**・**XLS-41**
  - XLS-27 は当面の対応（0 件テーブルを弾く番人）まで完了（`57c1b0d`）。本体修正待ち。
    マーカーカラム案の実測は未実施
  - **XLS-34〜XLS-41 は §6（中間モデル一巡点検）で新たに起こした 8 件のうちの要対応 7 件である**
    （残る XLS-39 は対応不要）。いずれも中間モデルの生成時拒否で閉じる。未着手
...
### XLS-35 識別子 `null` のデータブロックが、辺③では `SETUP_TABLE=null` として黙って書かれ、辺④ではスキーマ違反の `table: null` になる（影響度 中・**検出できない**）
