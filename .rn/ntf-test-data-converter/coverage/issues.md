...
課題は全部で **44 件**（課題 ID 単位）。内訳は **要対応 16 件**／**対応不要 27 件**／
...
> **現在は 全 44 件・要対応 16 ／ 対応不要 27 ／ 本作業の対象外 1 である**（冒頭の内訳を参照）。
...
上の出力は 2026-08-19 に実行して得たものである。`16 + 27 + 1 = 44` で 1 つ目の全件数と一致する。
...
- NTF 仕様としての判定: **本作業の対象外**（記録のみ。ユーザー確定・2026-08-18）。
  ...
  **上の 7 ／ 29 はこの記録を書いた時点（2026-08-18・全 37 件）の実数である。** その後 `7200b0f` で
  XLS-27〜XLS-33 の 7 件が加わり XLS-08 が要対応へ移ったため、**現在は 全 44 件・要対応 15 ／
  対応不要 28 ／ 本作業の対象外 1**（本項）である。**本項を二値に数えないことは変わっていない。**
