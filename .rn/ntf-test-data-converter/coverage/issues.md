...
変更はゼロである）。**#25.5 で方針が変わり、「NTF 仕様としての判定」が要対応とした 24 件のうち 23 件を
修正した**（2026-08-19 時点。steering Decisions「不具合修正の対象と手順（#25.5）」。
**残る 1 件は XLS-27 で、converter 側では閉じ切れず本体修正待ちである**）。修正済みの課題は各項の
...
課題は全部で **55 件**（課題 ID 単位）。内訳は **要対応 24 件**／**対応不要 28 件**／**保留 2 件**／
...
YML-05 から YML-14 として切り出したあとの値**）。
導出コマンドと出力は下記「集計は課題 ID 単位で数えること」の節にあり、
`24 + 28 + 2 + 1 = 55` が機械で確かめられる。

**要対応 24 件の内訳（修正済み 23 ／ 未完 1。2026-08-19 時点）**

- **修正済み 23 件** —— XLS-06・XLS-08・XLS-16・**XLS-20**・XLS-22・**XLS-28**・XLS-29・XLS-30・**XLS-31**・**XLS-32**・**XLS-33**・**XLS-34**・**XLS-35**・**XLS-36**・**XLS-37**・**XLS-38**・**XLS-40**・**XLS-41**・**XLS-43** ／
...
- **未完 1 件** —— XLS-27
  - XLS-27 は当面の対応（0 件テーブルを弾く番人）まで完了（`57c1b0d`）。**本体修正待ち**であり、
    converter 側で閉じ切れる課題ではない。
...
**対応不要 28 件のうち、2026-08-19 に 1 件増えた** —— **YML-14**（辺②でフィールド定義の件数より
要素数が多いデータ行の余りの値が黙って捨てられる）。**YML-05 から「多い側」を切り出したもの**で、
帰属は converter の外（本体 `DataFileFragment#addValue`）であり YML-10 と同じ他責の扱いである。
あるべき姿（`notation:891` に従い読み込みがエラーになる）のテストを `@Ignore` で残してある。
...
> | 番人 | 課題 | 決着 |
> |---|---|---|
> | `fileType` ＝ `null` | XLS-29 | **モデルへ移設**（`FileDataBlock` の生成時。§6-I 以前） |
...
上の出力は 2026-08-19 に実行して得たものである（**XLS-42・XLS-43・YML-14 を切り出したあとの値**）。
`24 + 28 + 2 + 1 = 55` で、1 つ目（`###` 見出しの数）とも 2 つ目（判定欄の行数）とも一致する。
