...
課題は全部で **52 件**（課題 ID 単位）。内訳は **要対応 23 件**／**対応不要 28 件**／
**本作業の対象外 1 件**（XLS-26）である。数え直した日は 2026-08-19（**§6 の中間モデル一巡点検で
XLS-34〜XLS-41 の 8 件を足したあとの値**）。導出コマンドと出力は下記
「集計は課題 ID 単位で数えること」の節にある。

**要対応 23 件の内訳（修正済み 17 ／ 未完 6。2026-08-19 時点）**
...
- **未完 6 件** —— XLS-27・**XLS-36**・**XLS-37**・**XLS-38**・**XLS-40**・**XLS-41**
  - XLS-27 は当面の対応（0 件テーブルを弾く番人）まで完了（`57c1b0d`）。本体修正待ち。
    マーカーカラム案の実測は未実施
  - **XLS-34〜XLS-41 は §6（中間モデル一巡点検）で新たに起こした 8 件のうちの要対応 7 件である**
    （残る XLS-39 は対応不要）。いずれも中間モデルの生成時拒否で閉じる
...
> **現在は 全 52 件・要対応 23 ／ 対応不要 28 ／ 本作業の対象外 1 である**（冒頭の内訳を参照）。
...
28
$ awk '/^### (XLS|YML)-/{id=$2} /^- NTF 仕様としての判定/{print id" "$0}' \
      .rn/ntf-test-data-converter/coverage/issues.md | grep -c '\*\*本作業の対象外\*\*'
1
```

上の出力は 2026-08-19 に実行して得たものである（**§6 の 8 件を足したあとの値**）。`23 + 28 + 1 = 52` で
1 つ目の全件数と一致する。
**4 つ目（`本作業の対象外`）は、3 つの区分の和が全件数に一致することを確かめるために足した**
（従来は 2 区分しかコマンドが無く、和が合うことを機械で確かめられなかった）。
...
| `TestDataBlock` | `groupId` | 括弧を値に含む（`[a]x[b]`） | 往復する | 別グループ `a]x[b` になる | **XLS-39**（対応不要） |
...
| `RecordLayout` | `fields` 件数と `rows` 各行の件数 | 不一致 | 読み戻しで例外 | そのまま書く | **XLS-41** |
| `RecordLayout` | `rows` | `null` | NPE | NPE | **XLS-38** |
| `FieldDef` | `name` ／ `type` | `null` | — | — | §1-D 済（XLS-31・XLS-30） |
| `FieldDef` | `length` | `null`（固定長・電文） | — | — | §1-G 済（XLS-30） |

**「無し」ではなかった。** 8 件（XLS-34〜XLS-41）を新規に起こした。うち **7 件が「NTF 仕様としての判定 ＝
要対応」**であり、XLS-39 だけが対応不要である。
...
- **括弧の無いグループ ID（`"G"`）** —— **XLS-11 と同一**（判定 **対応不要**）。`notation:266`
...
### XLS-39 グループ ID に記法の区切り文字（`[` ／ `]`）を含む中間モデルで、辺③④の指すグループが食い違う（影響度 低・記録のみ）
...
- 記法の明文: **無い。** `notation:278` は Excel 形式のグループ指定を `データタイプ名[グループID]=` と
  定め、`notation:266` は収集を `データタイプ + グループID + '='` の前方一致と定めるが、
  **グループ ID そのものに使える文字は定めていない**。`[` ／ `]` は記法が区切り文字として使っている
  文字であり、それを値に含む入力は記法の形（`データタイプ名[グループID]=`）を壊す。
- 判断: 記録に留める。**仕様外入力である**（XLS-01 と同じ扱い。記法が区切りに使う文字を値に含む入力を
  converter が正しく扱えることは、記法が求めていない）。
- NTF 仕様としての判定: **対応不要**（記法に明文が無い）。
- **方針 4**: 辺①は Excel マーカーの `データタイプ名` と `=` の間をそのまま切り出し、辺②は
  `formatGroup` が必ず `[` `]` で囲むため、**記法どおりの入力からこの値が生まれる経路はある**
  （Excel に `SETUP_TABLE[a]x[b]=T` と書けば作れる）。ただしそれは記法の形を壊した入力であり、
  リーダー側を直す対象ではない。
...
### XLS-41 フィールド定義の件数とデータ行の要素数が食い違う中間モデルが、辺③では読み戻しで例外になり、辺④ではそのまま書かれる（影響度 中・**辺④は検出できない**）
...
- 記法の明文: `notation:891`「データ要素数が不正である」（記述時エラーの一覧）。本体スキーマ
  `$defs.record_fragment.rows` も「各要素は fields と同順・同件数の値配列。…**各配列の要素数が fields の
  件数と一致しない場合は NTF がエラーを出す**」と定める。
- **どちらへ揃えるか**: **どちらでもない。生成時点で拒否する**（`RecordLayout` が
  `fields` の件数と各行の要素数の一致を保証する）。
- 判断: 仕様として不適切。
- NTF 仕様としての判定: **要対応**。
