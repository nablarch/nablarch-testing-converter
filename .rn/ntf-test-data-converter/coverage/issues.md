...
**要対応 22 件の内訳（修正済み 17 ／ 未完 5。2026-08-19 時点）**

- **修正済み 17 件** —— XLS-06・XLS-08・XLS-16・**XLS-20**・XLS-22・**XLS-28**・XLS-29・XLS-30・**XLS-31**・**XLS-32**・**XLS-33**・**XLS-34**・**XLS-35** ／
  YML-02・YML-03・YML-08・YML-12（**YML-12 は 4 形すべてが修正済み**）
- **未完 5 件** —— XLS-27・**XLS-36**・**XLS-37**・**XLS-38**・**XLS-40**
  - XLS-27 は当面の対応（0 件テーブルを弾く番人）まで完了（`57c1b0d`）。本体修正待ち。
    マーカーカラム案の実測は未実施
  - **XLS-36〜XLS-38・XLS-40 は §6（中間モデル一巡点検）で新たに起こした 8 件のうちの未着手分**である。
    いずれも中間モデルの生成時拒否で閉じる
...
### XLS-36 ブロックのクラスとデータ種別が不整合な中間モデルが、辺③では読み戻すと別種のブロックになり、辺④ではスキーマ違反の YAML になる（影響度 中・**辺③は検出できない**）
...
- 記法の明文: `notation:206-241` のデータタイプ ⇔ トップレベルキー対応表。**データタイプと、そのブロックが
  取りうる記述の形は 1:1 に対応する**（テーブル系・ファイル系・電文系）。本体スキーマも
  `table_data` ／ `file_data` ／ `message_data` を別の `$defs` として定義し、いずれも
  `additionalProperties: false` としている。
...
- **方針 4**: 辺①②のどちらのリーダーも不整合な組を作らない。上記のブロック生成箇所（XLS-34 参照）は
  **すべて正しいクラスと正しい `DataType` を対にして渡している**。したがって呼び出し側のバグである。
