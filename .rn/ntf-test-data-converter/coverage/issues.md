...
### XLS-40 カラム名・フィールド名が重複した中間モデルが、辺③では読み戻しで列が減り、辺④では同じキーを 2 回書く（影響度 中・**検出できない**）
...
- NTF 仕様としての判定: **要対応**。
- **方針 4**: 辺①は `XlsFormatReader#deduplicateColumnNames` が重複を除去してから中間モデルを作るため、
  **重複したカラム名を持つ中間モデルを作らない**（この除去自体は XLS-05 の関連挙動として記録済み）。
  辺②は YAML の Map から取るため構造上重複しない。したがって呼び出し側のバグである。
