...
2 通りある —— (a) **明文どうしが矛盾している**（XLS-41。`notation:883` と本体スキーマ
...
課題は全部で **52 件**（課題 ID 単位）。内訳は **要対応 22 件**／**対応不要 27 件**／**保留 2 件**／
**本作業の対象外 1 件**（XLS-26）である。数え直した日は 2026-08-19（**XLS-39 と XLS-41 を保留へ戻した
あとの値**）。導出コマンドと出力は下記「集計は課題 ID 単位で数えること」の節にあり、
`22 + 27 + 2 + 1 = 52` が機械で確かめられる。

**要対応 22 件の内訳（修正済み 21 ／ 未完 1。2026-08-19 時点）**

- **修正済み 21 件** —— XLS-06・XLS-08・XLS-16・**XLS-20**・XLS-22・**XLS-28**・XLS-29・XLS-30・**XLS-31**・**XLS-32**・**XLS-33**・**XLS-34**・**XLS-35**・**XLS-36**・**XLS-37**・**XLS-38**・**XLS-40** ／
...
**保留 2 件** —— **XLS-39**（グループ ID に使える文字の明文が無い）・**XLS-41**（`fields` 件数と行の
要素数について `notation:883` と本体スキーマが矛盾している）。**どちらも 2026-08-19 にユーザー指示で
判定を差し戻した**（XLS-39 は「対応不要」から、XLS-41 は「要対応」から）。実装は入れない。
XLS-39 は解説書担当宛の申し送りを立ててある（XLS-39 の節の直後）。
...
> さらに 2026-08-19 に **XLS-39** を「対応不要」から、**XLS-41** を「要対応」から**保留**へ戻したため、
> **現在は 全 52 件・要対応 22 ／ 対応不要 27 ／ 保留 2 ／ 本作業の対象外 1 である**（冒頭の内訳を参照）。
...
上の出力は 2026-08-19 に実行して得たものである（**XLS-39 と XLS-41 を保留へ戻したあとの値**）。
`22 + 27 + 2 + 1 = 52` で、1 つ目（`###` 見出しの数）とも 2 つ目（判定欄の行数）とも一致する。
...
| `RecordLayout` | `fields` 件数と `rows` 各行の件数 | 不一致 | 読み戻しで例外 | そのまま書く | **XLS-41**（保留） |
...
### XLS-41 フィールド定義の件数とデータ行の要素数が食い違う中間モデルが、辺③では読み戻しで例外になり、辺④ではそのまま書かれる（影響度 中・**辺④は検出できない**・**判定は保留**）
...
- **記法の明文とスキーマの明文が食い違っている（両方 2026-08-19 に実物で確認）。**「少ない側」について
  正反対のことを書いており、**converter 側で一方に寄せて実装してはならない。**

  | 出典 | 少ない側（行の要素数 < `fields` 件数）について何を定めているか |
  |---|---|
  | `notation:883` | 「データ行のセル数（Excel形式）または ``rows:`` の各要素の長さ（YAML形式）がフィールド数より少ない場合、不足したフィールドは ``""`` として補完される。**これを利用し、全フィールドを省略した行**（Excel形式では先頭セルが空の行、YAML形式では ``rows:`` に空配列 ``[]`` を記載した行）**を書けば、全フィールドが** ``""`` **のレコードとして保持される**」 —— **正常であり、記法として明示的に案内している** |
  | 本体スキーマ `$defs.record_fragment.rows` | 「各要素は fields と同順・同件数の値配列。…**各配列の要素数が fields の件数と一致しない場合は NTF がエラーを出す**」 —— **エラー** |

  **一致を強制すると、`notation:883` がわざわざ案内している書き方（`rows:` に `[]` を書いた行）で書かれた
  仕様適合データを、中間モデルが持てなくなる。**
- **どちらへ揃えるか**: **決められない。**明文どうしの矛盾であり、解説書側とスキーマ側の決着が要る。
- 判断: 辺③④の非対称（辺③は読み戻しで例外・辺④はそのまま書く）は事実として残るが、あるべき姿は
  明文の決着待ちである。
- NTF 仕様としての判定: **保留**（明文どうしが矛盾している）。**#25.5 では実装しない**
  （ユーザー確定・2026-08-19。§6-G を実装から記録へ切り替えた）。
- **【判定の訂正】2026-08-19（ユーザー指摘）。**「**要対応**」から **保留** へ変えた。旧判定は本体スキーマ
  `$defs.record_fragment.rows` の一文だけを根拠にしており、**それと正面から食い違う `notation:883` を
  見落としていた**。矛盾の一方だけを引いて要対応と判定したのが誤りである。
- **多い側（行の要素数 > `fields` 件数）は切り出せる。** こちらは `notation:891`「データ要素数が不正である」
  （ファイルデータの記述時エラー一覧）とスキーマが一致しており、**明文の矛盾が無い**。多い側だけに限定して
  課題を書き直せば要対応として扱える。**切り出すかはユーザー判断待ち**（2026-08-19 時点で未決。
  進行順の指示では §6-G は記録へ切り替えたため、本セッションでは切り出していない）。
- **方針 4**: 辺①②のどちらのリーダーも、フィールド定義行とデータ行を同じ表・同じ配列から読むため
  件数が食い違う中間モデルを作らない。呼び出し側のバグである。
