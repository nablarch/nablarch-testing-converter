...
**要対応 22 件の内訳（修正済み 20 ／ 未完 2。2026-08-19 時点）**

- **修正済み 20 件** —— XLS-06・XLS-08・XLS-16・**XLS-20**・XLS-22・**XLS-28**・XLS-29・XLS-30・**XLS-31**・**XLS-32**・**XLS-33**・**XLS-34**・**XLS-35**・**XLS-36**・**XLS-37**・**XLS-38** ／
  YML-02・YML-03・YML-08・YML-12（**YML-12 は 4 形すべてが修正済み**）
- **未完 2 件** —— XLS-27・**XLS-40**（**XLS-40 はフィールド名称側のみ修正済み。カラム名側が未実装で、扱いはユーザー判断待ち**）
  - XLS-27 は当面の対応（0 件テーブルを弾く番人）まで完了（`57c1b0d`）。本体修正待ち。
    マーカーカラム案の実測は未実施
  - **XLS-37・XLS-38・XLS-40 は §6（中間モデル一巡点検）で起こした 8 件のうちの未着手分**である。
    いずれも中間モデルの生成時拒否で閉じる
...
### XLS-40 カラム名・フィールド名が重複した中間モデルが、辺③では読み戻しで列が減り、辺④では同じキーを 2 回書く（影響度 中・**検出できない**・**#25.5 §6-F でフィールド名称側のみ修正済み。カラム名側は未実装**）
...
- 記法の明文:
  ...
  - テーブル系のカラム名: 本体スキーマ `$defs.table_data.rows` が各行を**オブジェクト**（キー＝カラム名）と
    定めていること自体が明文である。同じキーを 2 つ持つオブジェクトは書けない。
- **どちらへ揃えるか**: **どちらでもない。生成時点で拒否する**（`ColumnRowDataBlock` は `columnNames` の
  重複を、`RecordLayout` は `fields` の名称重複を拒否する）。
- 判断: **仕様として不適切**（辺③で列が黙って減り、辺④は読み戻せない YAML を作る）。
- NTF 仕様としての判定: **要対応**（**フィールド名称側のみ修正済み・2026-08-19**）。
...
- **カラム名側は実装していない（ユーザー判断待ち）。** `ColumnRowDataBlock` に同じ番人を置いて
  全テストを回したところ、`YamlFormatReaderInvalidInputTest#dropsValueWhenTableColumnNamesDifferOnlyByCase`
  が落ちた（実測 2026-08-19）。**辺②は重複したカラム名を持つ中間モデルを実際に作る** ——
  本体 `TableData` がテーブル系のカラム名を大文字化するため、`id` と `ID` を書いた**スキーマ適合の
  YAML** が `columnNames` ＝ `[ID, ID]` になる（**YML-10**）。生成時に拒否すると、NTF 自身は読める
  この入力を converter が変換できなくなる。
  - 選択肢 (a): 番人を入れ、YML-10 の入力を変換エラーにする（黙って値が消えるより早く気づけるが、
    仕様適合入力を変換できなくなる。原因は converter の外＝本体の大文字化にある）。
  - 選択肢 (b): 番人を入れず、辺②で「大文字化後に同名となるカラムがある」ことを検出して WARN する
    （YML-10 の「判断」欄が求めている扱い。辺①の F1-05 と同じ形になる）。
  - 選択肢 (c): カラム名側を別課題として切り出し、**保留**にする（本体側の決着待ち）。
