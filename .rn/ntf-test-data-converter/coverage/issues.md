...
#### §1-D（#25.5）の明文確認 —— 解説書の明文は個別要素の `null` まで届いていない（2026-08-18）
...
### XLS-32 グループ ID `null` のデータブロックが、辺③では `SETUP_TABLEnull=T` として黙って書かれ、辺④では手掛かりの無い `NullPointerException` になる（影響度 中・**辺③は検出できない**）
