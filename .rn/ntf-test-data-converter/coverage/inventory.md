...
**追補その 18（2026-08-29 実測。#46 で測定用の一時テストを削除したぶん）**

追補その 17 の ① は作業ツリーの値（687）で、追跡していない測定用の一時テスト 9 件
（`core/reader/ZzOracleProbeTest` ／ `xls/ZzProbeTest`）を含んでいた。#46 でこの 2 ファイルを削除したため、
**① と追跡対象の値が一致した。**
...
**追加も削除も無い**（一時テストは追跡していなかったため、追跡対象の件数 678 は変わらない）。
