...
> **#35 で 2 件とも削除した（2026-08-28）。**どちらの「あるべき姿」も現行の解説書（ピン `5783b35`）に
> 根拠が無かったためである（YML-14 は `notation:891` の記述時エラー一覧そのものが存在せず、
> XLS-40 のカラム名側はテーブルのカラム名の大小に触れる記述が 0 件。詳細は `issues.md` の各項）。
> **`@Ignore` は 0 件になり、`Skipped` も 0 になった**（`mvn -o clean test` → `Skipped: 0`）。
> 上の表と数値は `da66425` 時点の記録として残す（JaCoCo は再計測しない）。
