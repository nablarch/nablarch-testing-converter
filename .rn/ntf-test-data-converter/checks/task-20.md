...
次の 6 要素を再分類した。うち実 `.xlsx` を読ませた結果にもとづくのは C-11／C-13／C-16 の 3 件で、
C-17／C-20 は本体パーサのソース読解とプローブ、C-08 は「到達不能」ではなく「担保」への変更である。
...
### ラウンド 2・3（成果物 `4b745a2` 以降に対して実施・2026-08-12）

ラウンド 1 と同じく独立サブエージェントで起動し、**ラウンド 1 の判定・指摘は渡していない**（中立フレーミング）。
ユーザー指示により QA には「対応表・棚卸しの数字は、表の行を実際に足し合わせて検算すること」を必須観点として明示した。

| レビュー | ラウンド 2 の判定 | 指摘 | ラウンド 3（確認） |
|---|---|---|---|
| QA | **FAIL** | Major-1 §5.2 の陳腐化と自己矛盾／Major-2 「#18 では」の帰属誤り 3 箇所／Minor-3 C-17・C-20 に実行可能な根拠が無い／Minor-4 件数 14 と 17 の混在／Minor-5 無変更根拠の基点コミット違い／Minor-6 1 task = 1 commit からの逸脱 | `d9293bb` 後に **FAIL**（Major-2 が `checks/task-20.md` に 2 箇所残存、N-1〜N-3）→ `0811032` 後に **PASS**（新規指摘なし） |
| Craft (coding) | **PASS** | Minor 5 件（Javadoc のバッククォート／空判定の `assertTrue`／ヘルパ名 `assertBlock`／「唯一の索引」／姉妹クラス重複＝現状維持推奨） | `d9293bb` に対して **PASS**（新規指摘なし） |
| Verification (test) | **PASS** | Major-1 C-17・C-20 に実行可能な根拠が無い（QA Minor-3 と同一）／Minor 4 件 | `d9293bb` に対して **PASS**（アサーション置換 2 件を再ミューテーションし同値以上と確認） |

**ラウンド 2・3 の指摘への対応**（`d9293bb` ＋ `0811032`）:

- Major-2: `#18` は C-08／C-17 の担当タスクを指定していない（#18 版実物で確認）。誤った帰属を `inventory.md` 2 箇所・
  テスト Javadoc 1 箇所・`checks/task-20.md` 2 箇所で「本タスクの当初分類では」へ訂正
- Major-1: §5.2 を現時点ビューと明記し辺①列を ✅ へ更新（13/14・🔺 0）。冒頭案内も整合させた
- Verification Major-1 / QA Minor-3: C-17／C-20 の到達不能を F1-06 で実行可能な根拠にする作業を、
  steering #21 の Steps と Completion criteria に明記して送った（#20 では対応しない）
- Verification Minor-1: §1.2-2 軸A表を「`getDataType()` をアサート（＝担保）」と「同経路を通過（参考）」に分割。
  実コードから抽出した 10 メソッドと一致することを 2 レビュアーが独立に確認
- Craft Minor 1〜4 と QA Minor-4・Minor-5・N-1〜N-3 を反映

Minor-6（1 task = 1 commit からの逸脱。#20 は 7 コミット）は運用方針の問題としてユーザー判断に委ね、判定材料から外した。

## Overall Verdict

- Self-check: OK（修正ラウンド反映済み）
- QA: **OK**（ラウンド 3・`0811032` に対して PASS。新規指摘なし。全 354 テスト PASS・src/main 無変更・全表の検算一致を自ら実行して確認）
- Craft expert (coding): **OK**（ラウンド 2・`d9293bb` に対して PASS。新規指摘なし）
- Verification expert (test): **OK**（ラウンド 2・`d9293bb` に対して PASS。再ミューテーションで検証力の非退行を確認）
