...
## 追補 — #25.5 へ追加したステップぶんの self-check（2026-08-19）

対象は `steering.md` の `### #25.5` に**あとから足した**ステップである ——
§1-B〜§1-G ／ XLS-28 ／ §6（中間モデル一巡点検）／ §6-A〜§6-K ／ XLS-06・XLS-16 の決着 ／
台帳と件数の反映。当初の 5 件（`36e94a4`〜`6c8d90e`）ぶんの self-check は上の
「Self-check」節にあり、ここでは**繰り返さない**。

**実測の基準**: `HEAD` ＝ `451fe46`。
`JAVA_HOME=/usr/lib/jvm/temurin-17-jdk-amd64 mvn -o clean test -Djacoco.skip=true`
→ **`Tests run: 597, Failures: 0, Errors: 0, Skipped: 2`** ／ `BUILD SUCCESS`（2026-08-19 17:13 実行）。

### Completion criteria ごとの判定

| # | 条件 | 判定 | 根拠（実測） |
|---|---|---|---|
| 1 | 修正した課題それぞれについて、仕様どおりの期待値のテストが存在し、**修正前に赤になったこと（失敗メッセージ）が記録されている** | **NG（11 件）** | 下の「NG-1」に全件を挙げる |
| 2 | 現状挙動を固定していた既存テストが削除されている（同じ挙動を主張するテストが 2 本残っていない） | OK | 番人をモデルへ移した 5 ステップは、**空振りの緑になる書き出し側の番人テストを毎回削除した** —— §6-I 4 件・§6-J-1 2 件（＋主張が成り立たなくなった 1 件）・§6-J-2 4 件・§6-J-3 4 件・§6-K 2 件。各コミット本文に削除したテスト名を列挙してある |
| 3 | 1 件 ＝ 1 コミットになっている | **部分 NG（1 件）** | 下の「NG-2」 |
| 4 | ~~`@Ignore` は 0 件~~ → **他責の型の 2 件**であること | OK | `grep -rn '^    @Ignore' src/test --include=*.java` → **2 件**。`YamlFormatReaderInvalidInputTest.java:740`（YML-14。理由と他責先＝本体パーサ・出典 `notation:891` を引数に記載）／`:1277`（XLS-40 のカラム名側。他責先＝`nablarch-testing` の `TableData`） |
| 5 | XLS-01 のテストが削除されておらず、アサートが書き直されている | OK | 追加ステップでは XLS-01 に触れていない（当初 5 件ぶんの判定が有効） |
| 6 | ~~全 44 件・要対応 15／対応不要 28／対象外 1~~ → **全 55 件・要対応 25／対応不要 27／保留 2／対象外 1** が出典つきで書かれている | OK | 下の「導出コマンドの実測」。**区分外 0・二重 0**。「判断」欄は 1 件も取り消していない |
| 7 | YML-08 の「未確認」が実行結果で埋まっている | OK | 追加ステップでは触れていない（当初判定が有効） |
| 8 | `mvn clean test -Djacoco.skip=true` が全 PASS（~~`Skipped: 0`~~ → **`Skipped: 2`**） | OK | `Tests run: 597, Failures: 0, Errors: 0, Skipped: 2` ／ `BUILD SUCCESS` |
| 9 | `src/main` の変更が要対応と判定した課題の修正に必要な範囲に限られ、ファイル名・課題 ID・変更理由が記録されている | OK | 追加ステップの `src/main` 変更はすべて `fix(...)` コミット 1 つずつで、本文に課題 ID・明文の出典・変更理由がある。**他責の型の 3 件（XLS-40 カラム名側・YML-14・XLS-42）は `src/main` 無変更**（番人も WARN も無し）。**XLS-06・XLS-16 の決着（`999f41d`）は Javadoc のみで振る舞いを変えていない** |
| 10 | ~~辺③④の双方が弾く~~ → **中間モデルの生成時が弾く**（`FieldDef.type` ／ `MessageDataBlock.records`） | OK | `FieldDef.type` は §1-D `d0023c0`、`MessageDataBlock.records` は §6-J-2 `9e40644` でモデルへ寄せ、辺③④の番人は撤去した |
| 11 | `issues.md` の `notation:nnn` が全件 `30a8271` 基準であり、基準コミットが本文に書かれている | OK | `issues.md` に現れるコミット指定は `30a8271` 25 回（解説書）・`0197071` 2 回（本体スキーマ）・`65911f5` 3 回（`nablarch-testing` の `TableData`）で、**解説書の基準は `30a8271` だけ**である |
| 12 | ~~`RecordLayout` コンストラクタに番人を置かない判断が残っている~~ → **この判断は覆り、旧判断と却下理由は記録として残っている** | OK | 旧判断は `issues.md:1201` の引用ブロックに残る。現在 `RecordLayout` の生成時には番人が 3 つある（§6-F `29c9d1d` ／ §6-G の切り出し `166a199` ／ §6-I `c31b534`） |
| 13 | 本体パーサがレコード 0 件を受け付ける事実が新規 ID で記録され、判定が「本作業の対象外・記録のみ」である | OK | XLS-26 の判定欄 ＝ **本作業の対象外**（記録のみ。ユーザー確定・2026-08-18） |
| 14 | 中間モデルの全クラス・全フィールドの点検結果が記録されている | OK | `issues.md`「§6 中間モデル一巡点検（2026-08-19）」。**「無し」ではなく**新規 8 件（XLS-34〜XLS-41）を検出した |
| 15 | 課題 ID 単位の実数が、そのまま実行できる導出コマンド付きで確定している | OK | 下の「導出コマンドの実測」 |

### NG-1 —— 赤の失敗メッセージが記録されていない 11 件

**criterion 1 は満たしていない。** 追加ステップの `fix` コミット 18 件のうち、
**失敗メッセージそのものを記録しているのは 7 件だけ**である。

- **記録あり（失敗メッセージそのもの）7 件** —— §1-B `44469b2` ／ §1-C `3000baf`（どちらも
  上の追補に surefire 出力を貼付）、XLS-28 `5ab13d8`、§6-J-1 `7b0b381`、§6-J-2 `9e40644`、
  §6-J-3 `b762438`、§6-K `839bf64`（いずれもコミット本文）
- **件数だけで、メッセージが無い 2 件** —— §6-C `1244e2b`（先行する `94b0fbe` に
  `Tests run: 12, Failures: 3` とあるだけ）／ §6-G の切り出し `166a199`（「追加時点で前 2 本が
  RED であることを確認済み」とあるだけ）
- **記録が無い 11 件** —— §1-D `d0023c0` ／ §1-E `5abc773` ／ §1-F `81cf234` ／ §1-G `7c10654` ／
  §6-A `b905183` ／ §6-B `836a2a4` ／ §6-D `5803fe6` ／ §6-E `d87bc0b` ／ §6-F `29c9d1d` ／
  §6-H `7201650` ／ §6-I `c31b534`

**検査したのはコミット本文・`checks/task-25.5.md`・`steering.md`・`issues.md` の 4 か所で、
どこにも失敗メッセージが無い。**（`grep -i 'RED'` は `Co-Authored` の末尾 `red` を拾うため、
`Co-Authored-By` の行を除いて数えた。）

**赤を実行して確認していない、という意味ではない** —— 実装前にテストを書いて落ちることは
確かめている。**記録を残していない**のが欠落である。後追いで書き起こすと事後の作文になり、
`8c327d0` の QA レビューで「赤の独立再現で一字一句一致した」ことを担保にした水準を満たせないため、
**ここでは NG として開示するに留め、失敗メッセージの後付けはしない。**

### NG-2 —— 1 件 ＝ 1 コミットを守れなかった 1 件

`839bf64`（§6-K）が **XLS-27 と XLS-21 の 2 件**を含む。**分けられなかった** ——
先に XLS-27 の番人だけ外すと、カラム名を持たずセルを持つブロックが黙って誤って書き出される窓が開く。
逆に XLS-21 を先に入れると、撤去対象の番人テストが構築するブロックを新しいモデル側の番人が
先に落とし、コミット単位で緑にならない。理由はコミット本文と `steering.md` の §6-K に書いてある。

### 導出コマンドの実測（2026-08-19・`HEAD` ＝ `451fe46`）

```
$ F=.rn/ntf-test-data-converter/coverage/issues.md
$ grep -c '^### \(XLS\|YML\)-' $F                                        → 55
$ grep -c '^- NTF 仕様としての判定' $F                                    → 55
$ grep -c '^- NTF 仕様としての判定.*\*\*要対応\*\*' $F                     → 25
$ grep -c '^- NTF 仕様としての判定.*\*\*対応不要\*\*' $F                   → 27
$ grep -c '^- NTF 仕様としての判定.*\*\*保留\*\*' $F                       → 2
$ grep -c '^- NTF 仕様としての判定.*\*\*本作業の対象外\*\*' $F             → 1
$ grep '^- NTF 仕様としての判定' $F | grep -vc -E '\*\*(要対応|対応不要|保留|本作業の対象外)\*\*'   → 0
$ grep '^- NTF 仕様としての判定' $F | grep '\*\*要対応\*\*' | grep -cE '\*\*(対応不要|保留|本作業の対象外)\*\*' → 0
```

`25 + 27 + 2 + 1 = 55` で、見出しの数とも判定欄の行数とも一致する。**要対応 25 件はすべて修正済み。**

`inventory.md` の導出 ①②③ ——
①テストメソッド数 **597** ／ ②`@Ignore` **2 件** ／ ③`8c327d0: 536` ／ `HEAD: 597`。

### この追補について

`checks/` は git 管理下にあるが、**修正コミットには混ぜず単独の `docs` コミットで入れる**
（当初 5 件ぶんの self-check と同じ扱い）。**レビュア subagent には渡さない・読ませない**
（`steering.md:59` の取り決め）。QA 列・`## QA Expert Review`・`## Expert Reviews`・
`## Overall Verdict` には触れていない。
