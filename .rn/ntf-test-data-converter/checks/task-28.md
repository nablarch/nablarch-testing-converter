...
## 5. 持ち越し — 移動元が gc で消えうる

`1035207` は reflog からのみ到達可能である。`nablarch-testing` の設定は下記のとおりいずれも未設定＝既定値であり、
**到達不能コミットの reflog 保持は既定 30 日**、当該コミットは `2026-06-23` 作成で約 2 か月が経過している。
**`git gc` が走れば消える。**
...
救出用の参照（タグ・ブランチ）を本体に作るかは**ユーザー判断待ちであり、本作業では作っていない**
（ユーザー指示・2026-08-21）。本体の作業ツリーは `git status --short` が無出力＝クリーンであり、書き込みは行っていない。
