...
Excel のヘッダ行に同名のカラムが複数存在する場合、**変換は中断されずに続行**されます。

この挙動は後方互換を考慮した設計です。nablarch-testing 本体（NTF）は重複カラムを `TreeMap.put()` で暗黙に後勝ち上書きしてエラーなしに動作するため、コンバーターも同じ振る舞いを維持します。これにより、既存の Excel を修正せずにそのまま変換できます。

- **後勝ち**：同名カラムが複数あると、後方（右側）の列の値が採用されます。前方の列の値は破棄されます。これは NTF 実行時の挙動と同一です。
- **WARN ログ**：重複が検出されると `java.util.logging` の WARNING レベルでログが出力されます。ログにはファイル名・シート名・ブロック識別子・重複カラム名が含まれます。WARN を確認したら、元の Excel のヘッダ行を修正することを推奨します。
- **対象ブロック**：`LIST_MAP` ブロックと テーブル系ブロック（`SETUP_TABLE` / `EXPECTED_TABLE` / `EXPECTED_COMPLETED`）が対象です。

例として、以下のようなヘッダ行があった場合：

| COL_A | COL_B | COL_A |
|-------|-------|-------|
| first | b1    | last  |

変換後は `COL_A` は 1 列のみとなり、値は `last`（後勝ち）が採用されます。また以下のような WARN ログが出力されます：

```
[updateNormal.xlsx] シート "updateNormal" のブロック "MY_TABLE" に重複カラム名 "COL_A" があります。後方の列の値を採用します。
```
